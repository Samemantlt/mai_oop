...
TEST(TriangleTests, GetSizeValid) {
    EXPECT_DOUBLE_EQ(Triangle(Point<double>(0, 0), Point<double>(1, 0), Point<double>(0, 1)).get_size(), 0.5);
    EXPECT_DOUBLE_EQ(Triangle(Point(0, 0), Point(2, 0), Point(0, 2)).get_size(), 2);
}

TEST(TriangleTests, GetCenterValid) {
    EXPECT_EQ(Triangle(Point<double>(0, 0), Point<double>(1, 0), Point<double>(0, 1)).get_center(), Point(1.0/3, 1.0/3));
    EXPECT_EQ(Triangle(Point<double>(0, 0), Point<double>(3, 0), Point<double>(0, 3)).get_center(), Point<double>(1, 1));
}
